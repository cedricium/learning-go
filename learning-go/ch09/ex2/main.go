...
func ValidateEmployee(e Employee) error {
	if len(e.ID) == 0 {
		return FieldErr{Field: "ID"}
	}
	if !validID.MatchString(e.ID) {
		return ErrInvalidId
	}
	if len(e.FirstName) == 0 {
		return FieldErr{Field: "FirstName"}
	}
	if len(e.LastName) == 0 {
		return FieldErr{Field: "LastName"}
	}
	if len(e.Title) == 0 {
		return FieldErr{Field: "Title"}
	}
	return nil
}
